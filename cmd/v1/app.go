...
// App01 アプリケーションのテストデータを生成する
func (a *app) generateApp01(count int) error {
	appName := "app1"
	a.logger.Info("Generate test data", "app_name", appName)
	tableNameDataStructs := []TableNameDataStruct{
		{
			tableName:  userTableName,
			dataStruct: UserData{},
		},
	}
	return a.callGenerateCSVs(appName, tableNameDataStructs, count)
}

func (a *app) callGenerateCSVs(appName string, tableNameDataStructs []TableNameDataStruct, count int) error {
	// TODO: concurrent execution
	for _, v := range tableNameDataStructs {
		// analyze table data pattern and generate csv
		if err := a.generateCSV(appName, v.tableName, v.dataStruct, count); err != nil {
			return err
		}
	}
	return nil
...
func (a *app) generateCSV(appName, tableName string, dataStructure any, count int) error {
	// analyze table data pattern and generate csv
	a.logger.Info("Analyze table and given data pattern", "tableName", tableName, "count", count)
	resultCSV, err := a.analyzer.analyze(tableName, dataStructure, count)
	if err != nil {
	...
	csvOperator, err := csv.NewCSVGenerator(
		a.logger,
	...
	defer csvOperator.Close() //nolint:errcheck

	for _, v := range resultCSV {
		a.logger.Debug("result_csv_record", "resultCSV", v)
		if err := csvOperator.Generate(v...); err != nil {
